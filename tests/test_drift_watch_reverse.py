...
from evolution_utils import classify_orphan_issues, OrphanIssueClassification
...
def test_val_drf_002_orphan_with_session_evidence_closes():
    ...
    current_findings = [Finding("RULE_001", "warning", "test", "desc", "file1.py", "ev")]
    open_issues = [
        _make_issue(103, "RULE_003", "file3.py", 
...
def test_val_drf_002_orphan_no_evidence_retains_with_reason():
    ...
    with patch('evolution_utils._verify_fix_merged_via_linear', return_value=False), \
        ...
        assert orphan_104.reason != "", \
            "Must record blocking reason (audit trail requirement)"
...
def test_val_drf_002_three_reverse_sample_types():
    ...
    open_issues = [
        ...
        _make_issue(202, "RULE_202", "file202.py", 
    ...
    with patch('evolution_utils._verify_fix_merged_via_linear', side_effect=[True, True, False]), \
         patch('evolution_utils.subprocess.run') as mock_run:
        mock_run.return_value = MagicMock(returncode=0, stdout="", stderr="")
