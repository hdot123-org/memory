...
def test_pre_commit_includes_shellcheck():
    ...
    shellcheck_repos = [r for r in config["repos"] if "shellcheck" in r.get("repo", "").lower()]
...
def test_pre_commit_includes_actionlint():
    ...
    config = yaml.safe_load((REPO_ROOT / ".pre-commit-config.yaml").read_text())
    actionlint_repos = [r for r in config["repos"] if "actionlint" in r.get("repo", "").lower()]
...
def test_ci_runs_shellcheck():
    ...
    shell_lint_step = next((s for s in steps if s.get("name") == "Shell lint"), None)
...
def test_all_scripts_pass_shellcheck():
    ...
    sh_files = [
        ...
        if ".venv" not in f.parts and ".git" not in f.parts and "node_modules" not in f.parts
    ]
...
def test_ci_tarball_extraction_isolated():
    ...
    test_job = ci["jobs"]["test"]
    steps = test_job["steps"]
    install_step = next(s for s in steps if "Install shellcheck and actionlint" in s.get("name", ""))
    run_content = install_step["run"]
...
def test_ci_has_fix_has_test_guard():
    ...
    steps = test_job["steps"]
    guard_step = next(
        (s for s in steps if "fix-has-test" in s.get("name", "").lower() or "check_fix_has_test" in s.get("run", "")),
        None,
    )
...
def test_fix_has_test_guard_not_advisory():
    ...
    assert guard_step.get("continue-on-error") is not True, "Fix-has-test guard must not be continue-on-error"
...
def test_fix_has_test_guard_pr_only():
    ...
    assert "pull_request" in str(if_condition), f"Fix-has-test guard must be gated on pull_request, got: {if_condition}"
...
def test_fix_has_test_guard_uses_gh_token():
    ...
    assert "GH_TOKEN" in env or "GITHUB_TOKEN" in env, "Fix-has-test guard step must set GH_TOKEN or GITHUB_TOKEN"
    token_value = env.get("GH_TOKEN") or env.get("GITHUB_TOKEN")
    assert "secrets.GITHUB_TOKEN" in str(token_value), f"Token must reference secrets.GITHUB_TOKEN, got: {token_value}"
...
def _get_pr_ref_guard_step(ci):
    ...
    test_job = ci["jobs"]["test"]
    steps = test_job["steps"]
    return next((s for s in steps if "check_pr_ref_consistency" in s.get("run", "")), None)
...
def test_ci_has_pr_ref_consistency_guard():
    ...
    assert guard_step is not None, "PR reference consistency guard step not found in test job"
...
def test_pr_ref_consistency_guard_step_level_if_not_job_level():
    ...
    assert "pull_request" in if_condition, f"Guard must be gated on pull_request at step level, got: {if_condition}"
    # Must live in the test job, not a standalone job
    assert "test" in ci["jobs"]
    job_names = [
        name
        for name, job in ci["jobs"].items()
        if any("check_pr_ref_consistency" in s.get("run", "") for s in job.get("steps", []))
    ]
    assert job_names == ["test"], f"Guard must only exist in test job, found in: {job_names}"
...
def test_pr_ref_consistency_guard_uses_gh_token():
    ...
    env = guard_step.get("env", {})
    token_value = env.get("GH_TOKEN") or env.get("GITHUB_TOKEN")
    assert token_value is not None, "PR reference consistency guard must set GH_TOKEN"
    assert "secrets.GITHUB_TOKEN" in str(token_value), f"Token must reference secrets.GITHUB_TOKEN, got: {token_value}"
