...
def temp_lifecycle_root():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        path_index = {
            "paths": {
                "/tmp/proj1": {"git_root": "/tmp/proj1", "project_name": "proj1"},
                "/tmp/proj2": {"git_root": "/tmp/proj2", "project_name": "proj2"},
                "/tmp/proj1-dup-key": {  # different key, same git_root → registry dedup by git_root
                    "git_root": "/tmp/proj1",
        ...
        with (lifecycle_dir / "path-index.json").open("w") as f:
        ...
        proj3 = {"git_root": "/tmp/proj3", "project_name": "proj3", "status": "active"}
        with (projects_dir / "proj3.json").open("w") as f:
            json.dump(proj3, f)
        ...
        with (projects_dir / "proj4.json").open("w") as f:
            json.dump(proj4, f)
        ...
        with (projects_dir / "proj5.json").open("w") as f:
            json.dump(proj5, f)
...
def test_health_classification_active_kb(temp_lifecycle_root, temp_project_dirs):
    ...
    with (lifecycle_dir2 / "path-index.json").open("w") as f:
        json.dump(path_index, f)
