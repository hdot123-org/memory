...
def test_p1_safety_failed_category_not_closed():
    ...
    with (
        ...
        report = classify_orphan_issues(current_findings, open_issues, failed_categories=failed_categories)
...
def test_p1_safety_partial_output_skip_all_closing():
    ...
    mock_history_data = {
        "snapshots": [{"findings": [{"rule_id": f"R{i}", "location": f"f{i}"} for i in range(10)]} for _ in range(5)]
    }
    ...
    with (
        ...
        result = reverse_drift_watch(current_findings, open_issues, history_path=Path("/tmp/test_history.json"))
...
def test_p1_safety_all_three_guards_combined():
    ...
    open_issues = [
        _make_issue(701, "HEARTBEAT_STALE", "hb.py", category="evolution_self_audit", linear_linkback="INFRA-701"),
        _make_issue(702, "RULE_702", "f702.py", category="code_hygiene", linear_linkback="INFRA-702"),
        _make_issue(703, "RULE_703", "f703.py", linear_linkback="INFRA-703"),
    ]

    failed_categories = {"code_hygiene"}
    mock_history_data = {"snapshots": [{"findings": [{"rule_id": f"R{i}"} for i in range(10)]} for _ in range(5)]}
    ...
    with (
        ...
        assert result["closed"] == 0, f"Should close 0 issues with all guards. Closed {result['closed']}."
