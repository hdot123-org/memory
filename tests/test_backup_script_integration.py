...
class TestBackupScriptStructure:
    ...
    def test_has_dynamic_enumeration(self):
        ...
        # 检查是否处理枚举输出
        has_enumeration = (
            ("evolve_cli" in content or "memory-evolve" in content)
            and ("DYNAMIC_PATHS" in content or "while IFS= read" in content)
        )
    ...
    def test_has_enumeration_failure_handling(self):
        ...
        # 检查错误处理
        has_error_handling = (
            ("ENUM_FAILED" in content or "FAILED" in content)
            and ("WARN" in content or "ERROR" in content or "降级" in content)
        )
...
class TestBackupScriptCoverage:
    ...
    def test_dynamic_paths_cover_all_projects(self):
        ...
        try:
            ...
            active_projects = [
                p for p in projects
                if p.get("health") in ("active_kb", "no_kb")
            ]
