    ...
    def test_init_generates_all_runtime_reads(self, tmp_path: Path) -> None:
        ...
        for read_file in runtime_reads:
            ...
            if (
                ...
                and not any(read_file.startswith(d.replace("dir:", "")) for d in created_files if d.startswith("dir:"))
...
class TestInitGeneratesPolicyPack:
    ...
    def test_init_generates_policy_pack(self, tmp_path: Path) -> None:
        ...
        has_global_dir = any("memory/kb/global" in d or d == "memory/kb/global" for d in would_create_dirs)

        assert has_global_dir, "memory/kb/global directory should be created to allow policy-pack.json"
...
class TestInitGeneratesProjectScopeFile:
    ...
    def test_init_generates_project_scope_file(self, tmp_path: Path) -> None:
        ...
        has_projects_dir = any("memory/kb/projects" in d or d == "memory/kb/projects" for d in would_create_dirs)

        assert has_projects_dir, f"memory/kb/projects directory should be created for host={host}"
        ...
        assert len(projects_dirs) > 0, f"Projects directories should be created for host={host}"
...
class TestInitGeneratesInboxMd:
    ...
    def test_init_generates_inbox_md(self, tmp_path: Path) -> None:
        ...
        assert "memory/inbox.md" in created_files, "memory/inbox.md should be generated by init"
...
class TestInitRuntimeCoverageComprehensive:
    ...
    def test_init_creates_all_essential_directories(self, tmp_path: Path) -> None:
        ...
        for essential in essential_dirs:
            assert essential in would_create_dirs, f"Essential directory {essential} should be created by init"
    ...
    def test_init_creates_adapter_toml(self, tmp_path: Path) -> None:
        ...
        assert "adapter.toml" in created_files, "adapter.toml should be generated by init"
