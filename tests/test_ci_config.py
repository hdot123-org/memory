...
class TestBranchCleanupNamingContract:
    ...
    def test_workflow_name_exact(self, branch_cleanup_data):
        ...
        assert branch_cleanup_data["name"] == "Branch Cleanup", \
            f"workflow 名被修改: {branch_cleanup_data['name']}"
    ...
    def test_triggers_preserved(self, branch_cleanup_data):
        ...
        assert schedule_list[0].get("cron") == "0 * * * *", \
            f"cron 表达式被修改: {schedule_list[0].get('cron')}"
    ...
    def test_dispatch_token_forwarded(self, branch_cleanup_data):
        ...
        assert "secrets.DISPATCH_TOKEN" in with_block["dispatch-token"], \
            "dispatch-token 未引用 secrets.DISPATCH_TOKEN"
