...
class TestGateSetupLabelsGovernance:
    ...
    def test_setup_labels_is_thin_caller(self):
        ...
        assert "hdot123-org/infra-core/.github/workflows/setup-labels.yml@v0.15.1" in uses_str
    ...
    def test_governance_uses_infra_core_composite_action(self):
        ...
        assert "hdot123-org/infra-core/actions/governance-check@v0.15.1" in uses_str
...
class TestAuditGate:
    ...
    def test_val_gate_002_review_shard_uses_matrix(self, droid_review_data):
        ...
        caller 必须委托该 workflow（uses 字节级指向 droid-review-shards.yml@v0.15.1）。"""
        shards_job = droid_review_data["jobs"]["shards"]
        uses_str = shards_job.get("uses", "")
        assert uses_str == "hdot123-org/infra-core/.github/workflows/droid-review-shards.yml@v0.15.1", (
    ...
    def test_val_gate_006_auto_merge_no_bypass(self):
        ...
        assert jobs["auto-merge"].get("uses") == (
            "hdot123-org/infra-core/.github/workflows/auto-merge-pipeline.yml@v0.15.1"
        )
...
class TestAutoMergeThinCallerContract:
    ...
    def test_single_job_delegation_with_event_guard(self, auto_merge_data):
        ...
        job = jobs["auto-merge"]
        assert job.get("uses") == "hdot123-org/infra-core/.github/workflows/auto-merge-pipeline.yml@v0.15.1"
...
class TestScanHeartbeatThinCallerContract:
    ...
    def test_scan_single_job_delegation(self, scan_data):
        ...
        INFRA-651: uses 钉 tag v0.15.1（与 pyproject 引擎 pin 同版本）——禁浮动
        @main，钉 @v0.15.1；浮动期间 infra-core 模板先行漂移会让本仓定时管道
        ...
        job = jobs["scan"]
        assert job.get("uses") == "hdot123-org/infra-core/.github/workflows/evolution-scan.yml@v0.15.1"
        assert job.get("steps") is None
    ...
    def test_heartbeat_single_job_delegation(self, heartbeat_data):
        ...
        INFRA-651: uses 钉 tag v0.15.1，与 scan caller 及 pyproject 引擎 pin
        同版本（模板/引擎版本锁定，禁浮动 @main，钉 @v0.15.1）。
        """
        jobs = heartbeat_data["jobs"]
        assert list(jobs.keys()) == ["heartbeat"]
        job = jobs["heartbeat"]
        assert job.get("uses") == "hdot123-org/infra-core/.github/workflows/evolution-heartbeat.yml@v0.15.1"
        assert job.get("steps") is None
...
class TestWatchdogThinCallerContract:
    ...
    def test_handler_jobs_delegate_to_infra_core(self, watchdog_data):
        ...
        expected = "hdot123-org/infra-core/.github/workflows/droid-review-watchdog-handlers.yml@v0.15.1"
