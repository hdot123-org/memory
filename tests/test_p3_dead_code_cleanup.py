...
class TestClaudeHookStateDirDeadCode:
    ...
    def test_no_claude_hook_state_dir_in_memory_core(self) -> None:
        ...
        if result.returncode == 0:
            ...
            files = [
                ...
                if line.strip() and "/archive/" not in line and "/docs/" not in line
            ]
            assert not files, f"CLAUDE_HOOK_STATE_DIR still found in memory_core source: {files}"
    ...
    def test_no_claude_hook_state_dir_in_gateway_module(self) -> None:
        ...
        if gateway_path.exists():
            ...
            assert "CLAUDE_HOOK_STATE_DIR" not in content, "CLAUDE_HOOK_STATE_DIR should not appear in gateway module"
