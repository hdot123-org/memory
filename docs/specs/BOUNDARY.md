...
| Evolution 引擎与审计工具执行体 | `infra-core` 仓 | 自 M3/M5 起，scanner / heartbeat / daily-audit / error-patterns / self-audit / hygiene / layout-audit / version-sync 的执行体与 CLI 入口（`infra-*`）在 infra-core；本仓仅保留协议面、`.evolution/config.yml` 消费配置（rule_packs）。回滚副本（`scripts/evolution_{scanner,heartbeat,utils,adapters}.py`）已随 PR #1097 删除（M1 F2 引擎副本清尸，回滚窗口关闭） |
