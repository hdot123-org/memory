...
## 架构 (v0.49.0) <!-- x-release-please-version -->
...
## 遥测架构 (v0.49.0) <!-- x-release-please-version -->
...
```bash
pip install git+https://github.com/hdot123-org/memory.git@v0.49.0 <!-- x-release-please-version -->
```

升级到新版本：

```bash
pip install --upgrade git+https://github.com/hdot123-org/memory.git@v0.49.0 <!-- x-release-please-version -->
```

从 release wheel 安装：

```bash
gh release download v0.49.0 --repo hdot123-org/memory --pattern "*.whl" <!-- x-release-please-version -->
pip install memory_core-0.49.0 <!-- x-release-please-version -->
```
...
分支清理执行体已迁至 infra-core：本仓 `.github/workflows/branch-cleanup.yml` 为 thin caller（`uses: hdot123-org/infra-core/actions/branch-cleanup`，tag pin 当前 v0.15.2），仓库内不再持有 `scripts/branch_cleanup*.sh` 副本。`--scheduled`（每小时扫描孤立分支）与 `--immediate <branch>`（PR 合并后定点清理）两种模式由 composite action 提供。
...
- 当前文档版本：v0.49.0 <!-- x-release-please-version -->
- Python: >= 3.9
