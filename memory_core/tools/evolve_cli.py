...
def _get_last_run_stats(evolution_root: Path) -> dict:
    """
...
def main(argv: Optional[List[str]] = None):
    ...
    status_parser = subparsers.add_parser(
        "status",
        help="列出全部注册项目 + 健康分类 + 上次运行统计"
    )
    status_parser.add_argument(
        "--json",
        action="store_true",
        help="输出 JSON 格式（D6: stdout 只放载荷）"
    )
    
    # backup-paths 子命令
    backup_parser = subparsers.add_parser(
        "backup-paths",
        help="输出全部消费项目 memory 目录路径（D11: missing 剔除）"
    )
